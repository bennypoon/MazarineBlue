...
public abstract class Picture {
    ...
    public Picture clip(Dimension dimension)
        ...
        Raster r = new Raster(dimension.width, dimension.height);
        r.copyAndClip(this.raster);
        return createPicture(r);
    }
    ...
    private boolean isThisImageSmallerOrEqual(Dimension dimension) {
        return raster.width <= dimension.width && raster.height <= dimension.height;
    }
    // </editor-fold>
    ...
    public final EqualCompounder.Result compare(Picture right, PixelComperator pixelComperator)
            throws IOException {
        if (right == null)
            return EqualCompounder.Result.FAIL;
        EqualCompounder check = new EqualCompounder(raster, right.raster, pixelComperator);
    ...
    public final Picture compound(Picture right, CompoundMethod method)
        ...
        ScreenshotsCompounder compounder = new ScreenshotsCompounder(raster, right.raster, method);
        compounder.computeScreenshot();
        Raster result = compounder.getComperation();
        return createPicture(result);
    }

    protected abstract Picture createPicture(Raster raster)
            throws IOException;
}
